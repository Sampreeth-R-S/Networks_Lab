...
{   
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            struct ethhdr *eth = (struct ethhdr *)response_packet;
            struct iphdr *ip = (struct iphdr *)(response_packet + sizeof(struct ethhdr));
            if (ntohs(eth->h_proto) != ETH_P_IP || ip->protocol != 254) {
                //printf("Received packet is not aDNS response packet. Discarding.\n");
                continue;
            }
            simDNS_ResponsePacket *response = (simDNS_ResponsePacket *)(response_packet + sizeof(struct ethhdr) + sizeof(struct iphdr));
            if (response->id != query.id) {
                //printf("Received packet does not match the query ID. Discarding.\n");
                continue;
            }
        ...
        printf("Timeout\n");
        bytes_sent = send(sockfd, packet, packet_size, 0);
        if (bytes_sent < 0) {
            perror("send");
            close(sockfd);
            //free(packet);
